...
function Cart() {
    ...
    const handleCheckoutWithPayment = async () => {
        ...
        createTransaction(amount).then(order => {
            handlePayment(order)
        }).catch(error => {
            console.log(error)
        })
    ...
    const handleCheckoutWithoutPayment = async () => {
        const orderRequest = { cartId: cart.cartId, orderAddress: address }
                        ...
                        <>
                            <div>
                                <Form className='my-3'>
                                    ...
                                    <Input type='text' id='address' onChange={(event) => handleAddressChange(event)} value={address || ''}></Input>
                                    {address === "" && <span style={{ color: "red", marginLeft: "0px", marginTop: "5px", fontSize: "12px" }} className='text-center'>Address cannot be empty</span>}
                                </Form>
                            </div>
